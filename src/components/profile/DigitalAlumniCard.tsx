...
export const DigitalAlumniCard: React.FC<DigitalAlumniCardProps> = ({ user: propUser, compact = false }) => {
  ...
  const cardFront = (
    <div className="relative w-full h-full rounded-2xl overflow-hidden bg-gradient-to-br from-[#800000] via-[#520000] to-[#260000] text-white p-5 sm:p-6 flex flex-col justify-between shadow-2xl border border-amber-500/30 select-none">
      ...
      <div className="relative z-10 flex items-start justify-between">
        <div className="flex items-center gap-3">
          <img
            ...
            className="w-10 h-10 rounded-full border-2 border-amber-400/80 shadow-md object-cover bg-white shrink-0"
          />
          <div>
            <span className="text-[10px] tracking-widest text-amber-300 font-semibold uppercase block">
              St. Cecilia’s College - Cebu
            </span>
            <span className="text-xs sm:text-sm font-black tracking-wide text-white drop-shadow-xs block">
              ALUMNI DIGITAL PASS
            </span>
          </div>
        </div>
      ...
      <div className="relative z-10 flex items-center justify-between my-auto py-2">
        ...
        <div className="w-11 h-8 rounded-md bg-gradient-to-tr from-amber-400 via-amber-200 to-amber-500 border border-amber-600 shadow-inner flex flex-col justify-around p-1">
          <div className="w-full h-0.5 bg-amber-700/50 rounded-xs" />
          <div className="flex justify-between">
            <div className="w-3 h-2 border border-amber-700/50 rounded-xs" />
            <div className="w-3 h-2 border border-amber-700/50 rounded-xs" />
          </div>
          <div className="w-full h-0.5 bg-amber-700/50 rounded-xs" />
        </div>
        ...
        <div className="flex items-center gap-2 bg-black/40 backdrop-blur-xs p-1.5 rounded-lg border border-amber-400/30">
          <QrCode className="w-8 h-8 text-amber-200" />
      ...
      <div className="relative z-10 my-1 flex items-center justify-between">
        <span className="font-mono text-base sm:text-lg tracking-widest text-amber-100 font-bold drop-shadow-sm">
          {maskedId}
        </span>
        <button
          type="button"
          ...
          className="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded bg-black/30 hover:bg-black/50 text-amber-200 border border-amber-400/20 transition-colors"
        >
      ...
      <div className="relative z-10 pt-2 border-t border-amber-500/20 flex items-end justify-between">
        <div className="min-w-0 pr-2">
          <div className="flex items-center gap-1.5">
            ...
            {user.isVerified && (
              <ShieldCheck className="w-4 h-4 text-amber-400 shrink-0" title="Verified Graduate" />
            )}
          </div>
          <span className="text-[10px] text-amber-200/90 tracking-wide truncate block">
            {user.course || 'Alumni Member'}
          </span>
        </div>

        <div className="text-right shrink-0">
          <span className="text-[9px] text-amber-300/80 uppercase tracking-widest block font-medium">
            CLASS / STATUS
          </span>
  ...
  return (
    <>
      <div className={`flex flex-col items-center ${compact ? 'max-w-md w-full' : 'max-w-lg w-full'} mx-auto`}>
        ...
        {/* Card Action Toolbar */}
        <div className="flex items-center justify-between w-full mt-3 px-1 text-xs text-stone-600">
          <button
            type="button"
            onClick={() => setIsFlipped(!isFlipped)}
            className="flex items-center gap-1.5 py-1.5 px-3 rounded-lg bg-stone-100 hover:bg-stone-200 text-stone-700 font-semibold transition-colors"
          >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 py-1.5 px-3 rounded-lg bg-stone-100 hover:bg-stone-200 text-stone-700 font-medium transition-colors"
