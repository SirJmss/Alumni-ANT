                ...
                <div className="p-4 pt-3 bg-stone-50/60 border-t border-stone-100">
                  ...
                  <div className="pt-2 border-t border-stone-100 flex flex-col gap-2">
                    <div className="flex items-center justify-between text-[11px]">
                      <span className="text-stone-500 font-medium">Your Attendance:</span>
                      <span className={`font-semibold flex items-center gap-1 ${
                        ...
                        {evt.userRsvp === 'going' && <Check className="w-3 h-3 text-emerald-600" />}
                        {evt.userRsvp === 'going'
                          ? 'Confirmed Attending'
                          : evt.userRsvp === 'interested'
                    ...
                    <div className="grid grid-cols-3 gap-1.5">
                      <button
                        ...
                        title={evt.userRsvp === 'going' ? 'Click to cancel attendance' : 'RSVP: I am Going'}
                        className={`py-2 px-2 text-xs font-semibold rounded-lg flex items-center justify-center gap-1 transition-all ${
                          evt.userRsvp === 'going'
                      ...
                      >
                        <Check className={`w-3.5 h-3.5 ${evt.userRsvp === 'going' ? 'text-white' : 'text-stone-400'}`} />
                        <span>{evt.userRsvp === 'going' ? 'RSVP: Going' : 'Going'}</span>
                      </button>

                      <button
                        type="button"
                        onClick={() => rsvpEvent(evt.id, 'interested')}
                        title={evt.userRsvp === 'interested' ? 'Click to cancel' : 'RSVP: I am Interested'}
                        className={`py-2 px-2 text-xs font-medium rounded-lg flex items-center justify-center gap-1 transition-all ${
                          evt.userRsvp === 'interested'
                            ? 'bg-amber-600 text-white shadow-xs ring-2 ring-amber-500/30'
                            : 'bg-stone-50 hover:bg-stone-100 border border-stone-200 text-stone-700'
                        }`}
                      >
                        <Sparkles className={`w-3.5 h-3.5 ${evt.userRsvp === 'interested' ? 'text-white' : 'text-stone-400'}`} />
                      ...
                      <button
                        type="button"
                        onClick={() => setSelectedEventForDetail(evt)}
                        className="py-2 px-2 text-xs font-medium bg-stone-100 hover:bg-stone-200 text-stone-700 rounded-lg text-center transition-colors"
                      >
                        Details
                      </button>
                    </div>
