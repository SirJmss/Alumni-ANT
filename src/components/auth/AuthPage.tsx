...
export const AuthPage: React.FC<AuthPageProps> = ({
  ...
  // Quick screening sample IDs for instant screening demonstration
  const quickScreeningIds = [
    { id: 'SC-2020-0192', name: 'Juan Dela Cruz (BSIT)' },
    { id: 'SC-2016-0812', name: 'Maria Santos (BSBA)' },
    { id: 'SC-2022-0104', name: 'Angelo Diaz (BSCpE)' },
    { id: 'SC-2014-0105', name: 'Maria Santos (BSEd)' },
    { id: 'SC-2018-0554', name: 'Paolo Mendoza (BSHM)' }
  ];

  // Step 1 Screening Verification Handler
  const handleVerifyStudentClick = async (e?: React.MouseEvent | React.FormEvent, overrideId?: string) => {
    if (e && 'preventDefault' in e) e.preventDefault();
    const targetId = (overrideId || studentId).trim();
    if (!targetId) {
      setScreeningError('Please enter your St. Cecilia’s Student ID (e.g. SC-2020-0192) to verify.');
      return;
    }
    ...
    try {
      ...
      if (regMatch.isMatched && regMatch.record) {
        ...
        setVerificationMessage(
          `Official Registrar Masterlist Match! Pre-accredited graduate record verified for ${regMatch.record.fullName} (${regMatch.record.course}, Class of ${regMatch.record.batchYear}). Instant alumni auto-registration activated!`
        );
        if (regMatch.record.fullName) {
          const parts = regMatch.record.fullName.trim().split(' ');
          if (parts.length > 1) {
            setFirstName(parts.slice(0, -1).join(' '));
            setLastName(parts[parts.length - 1]);
          } else {
            setFirstName(regMatch.record.fullName.trim());
          }
        }
        if (regMatch.record.course) setCourse(regMatch.record.course);
        if (regMatch.record.batchYear) setBatch(regMatch.record.batchYear);
        if (regMatch.record.email && !regEmail) setRegEmail(regMatch.record.email);
        return;
      }
      ...
      if (result.isVerified) {
        ...
        if (result.record) {
          ...
          if (result.record.fullName && result.record.fullName !== 'Verified Cecilian Alum') {
            const parts = result.record.fullName.trim().split(' ');
            if (parts.length > 1) {
              setFirstName(parts.slice(0, -1).join(' '));
              setLastName(parts[parts.length - 1]);
            } else {
              setFirstName(result.record.fullName.trim());
            }
          }
          if (result.record.course) setCourse(result.record.course);
          if (result.record.batchYear) setBatch(result.record.batchYear);
        }
      } else {
  ...
  // Step 1 Screening Validation -> Proceed to Step 2 Account & Profile
  const handleProceedFromStep1ToStep2 = async (e?: React.FormEvent) => {
    ...
    if (!targetId) {
      setScreeningError('Please enter your St. Cecilia’s Student ID Number (e.g. SC-2020-0192) to proceed.');
      return;
    }

    // If already verified, advance directly
    if (studentVerificationStatus === 'verified') {
      setStep(2);
      return;
    }

    // If not yet verified, run real-time screening check
    setStudentVerificationStatus('checking');

    // 1. Try Registrar Masterlist match
    const regMatch = findRegistryMatch({ studentId: targetId });
    if (regMatch.isMatched && regMatch.record) {
      setStudentVerificationStatus('verified');
      setIsRegistryMatched(true);
      setVerifiedRecord(regMatch.record);
      setVerificationMessage(
        `Official Registrar Masterlist Match! Pre-accredited graduate record verified for ${regMatch.record.fullName}.`
      );
      if (regMatch.record.fullName) {
        const parts = regMatch.record.fullName.trim().split(' ');
        if (parts.length > 1) {
    ...
    if (isValidStudentIdPattern(targetId)) {
      const result = await verifyStudentRecord({ studentId: targetId });
      if (result.isVerified) {
        setStudentVerificationStatus('verified');
        setIsRegistryMatched(false);
        setVerificationMessage(result.message);
    ...
    setStudentVerificationStatus('failed');
    setScreeningError(
      'Could not verify Student ID. Please ensure format is SC-YYYY-XXXX (e.g. SC-2020-0192) or contact the Registrar.'
    );
  };
  ...
  const handleCompleteRegistration = () => {
    ...
    const finalStudentId = studentId.trim() || `SC-${batch}-${Math.floor(1000 + Math.random() * 9000)}`;
          ...
          <div className="max-w-md w-full mx-auto">
            {registrationComplete ? (
              ...
              <>
                ...
                {/* STEP 1: INITIAL ACADEMIC SCREENING VIA STUDENT ID */}
                {step === 1 && (
                  <div>
                    <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider bg-red-100 text-[#8B181B] mb-2 border border-red-200">
                      ...
                      Step 1 of 3 • Initial Academic Screening
                    </div>
                    ...
                    <p className="text-sm text-stone-500 font-normal mb-5">
                      To preserve alumni community integrity, please enter your official St. Cecilia’s Student ID Number for preliminary academic verification.
                    </p>
                    ...
                    <form onSubmit={handleProceedFromStep1ToStep2} className="space-y-4">
                      ...
                      <div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-2">
                          ...
                          <button
                            ...
                            className="px-4 py-2.5 bg-stone-900 hover:bg-stone-800 disabled:opacity-50 text-white rounded-xl text-xs font-bold transition-all shadow-xs flex items-center justify-center gap-1.5 shrink-0 cursor-pointer"
                          >
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2 text-xs font-bold">
                              ...
                              <span>
                                {isRegistryMatched
                                  ? 'Official Registrar Match • Pre-Accredited Graduate'
                          ...
                          {verifiedRecord && (
                            <div className="pt-2 border-t border-stone-200/60 grid grid-cols-2 gap-2 text-xs">
                              <div>
                                <span className="text-[10px] text-stone-400 uppercase block">Student Name</span>
                                <span className="font-bold text-stone-900">{verifiedRecord.fullName}</span>
                              </div>
                              <div>
                                <span className="text-[10px] text-stone-400 uppercase block">Graduation Batch</span>
                                <span className="font-bold text-stone-900">Class of {verifiedRecord.batchYear}</span>
                              </div>
                              <div className="col-span-2">
                                <span className="text-[10px] text-stone-400 uppercase block">Degree Program</span>
                                <span className="font-semibold text-stone-800">{verifiedRecord.course}</span>
                              </div>
                            </div>
                          )}
                        </div>
                          ...
                          {quickScreeningIds.map((item) => (
                            <button
                              ...
                              onClick={() => {
                                ...
                                handleVerifyStudentClick(undefined, item.id);
                              }}
                              className="px-2.5 py-1 text-[11px] font-medium bg-white hover:bg-red-50 hover:border-[#8B181B] text-stone-700 hover:text-[#8B181B] border border-stone-200 rounded-lg transition-colors flex items-center gap-1 cursor-pointer"
                            >
                              <span className="font-mono font-bold text-[#8B181B]">{item.id}</span>
                              <span className="text-stone-400 text-[10px]">({item.name})</span>
                            </button>
                          ))}
                        </div>
                      </div>
