...
export const AlumniProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const rsvpEvent = (eventId: string, status: 'going' | 'interested' | 'not_going') => {
    ...
    // Send automated Event Registration Confirmation notification
    if (newStatus !== 'not_going') {
      const confirmNotif: AppNotification = {
        id: `notif_rsvp_${Date.now()}`,
        type: 'event',
        title: 'Event Registration Confirmed!',
        body: `You are confirmed as ${newStatus === 'going' ? 'attending' : 'interested in'} "${targetEvent.title}". We look forward to seeing you.`,
        read: false,
        createdAt: new Date().toISOString(),
        metadata: { eventId: targetEvent.id }
      };
      ...
      addAuditLog({
        ...
        details: `Alumnus confirmed RSVP (${newStatus}) for event: "${targetEvent.title}".`,
        severity: 'info'
      });
    } else {
      addAuditLog({
